...
export default (Comp)=>{
    ...
    withReduxApp.getInitialProps = async (appContext) =>{
        ...
        let resp = await axios.get(`http://localhost:8080/liner-back/files/TY_LINER/cache/Enum.js`);
        if(resp.status == 200 && resp.data){
            remoteState['enum'] = resp.data
        }
        resp =await axios.get('http://localhost:8080/liner-back/api/WebApi/init');
